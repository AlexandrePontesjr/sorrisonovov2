...
interface WhatsAppButtonProps {
  ...
  intent?: 'whatsapp' | 'schedule';
  className?: string;
  children?: ReactNode;
}

const WHATSAPP_LINK = 'https://wa.me/5592981887734?text=Olá!%20Gostaria%20de%20agendar%20uma%20consulta.';
const SCHEDULING_LINK = 'https://clinicasorrisonovo.codental.site/marcar-consulta';
...
export function WhatsAppButton({
  ...
  const href = isSchedule ? SCHEDULING_LINK : WHATSAPP_LINK;
  const label = isSchedule ? 'Agendar consulta' : 'Fale conosco pelo WhatsApp';
  const icon = isSchedule ? <CalendarDays className="h-[18px] w-[18px]" aria-hidden="true" /> : <WhatsAppIcon />;
